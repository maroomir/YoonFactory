...
namespace YoonFactory.Cognex
{
    public static class CognexFactory
    {
        ...
        public static CogImage8Grey SetMonoImage(IntPtr pBufferAddress, int nWidth, int nHeight)
        {
            ...
            return SetMonoImage(pBuffer, nWidth, nHeight);
        }

        public static CogImage8Grey SetMonoImage(byte[] pBuffer, int nWidth, int nHeight)
        {
        ...
        public static CogImage24PlanarColor SetColorImageMixed(IntPtr pBufferAddress, int nWidth, int nHeight, int nPlane=3, bool bRGBOrder=true)
        {
            if (pBufferAddress == IntPtr.Zero) return null;
            if (nPlane < 3) return null;

            byte[] pBuffer = new byte[nWidth * nHeight * nPlane];
            Marshal.Copy(pBufferAddress, pBuffer, 0, nWidth * nHeight * nPlane);
            return SetColorImageMixed(pBuffer, nWidth, nHeight, nPlane, bRGBOrder);
        }

        public static CogImage24PlanarColor SetColorImageParallel(IntPtr pBufferAddress, int nWidth, int nHeight, int nPlane = 3, bool bRGBOrder = true)
        {
            if (pBufferAddress == IntPtr.Zero) return null;
            if (nPlane < 3) return null;
            ...
            byte[] pBuffer = new byte[nWidth * nHeight * nPlane];
            Marshal.Copy(pBufferAddress, pBuffer, 0, nWidth * nHeight * nPlane);
            return SetColorImageParallel(pBuffer, nWidth, nHeight, nPlane, bRGBOrder);
        }

        public static CogImage24PlanarColor SetColorImageMixed(byte[] pBuffer, int nWidth, int nHeight, int nPlane = 3, bool bRGBOrder = true)
        {
            if (pBuffer == null || pBuffer.Length != nWidth * nHeight * nPlane) return null;
            if (nPlane != 3) return null;
            ...
            try
            {
                ICogImage8PixelMemory[] cogMemoryPlaneArray = new ICogImage8PixelMemory[nPlane];
                cogImage.Allocate(nWidth, nHeight);
                ...
                for (int iByte = 0; iByte < nWidth * nHeight * nPlane; iByte++)
                {
                    //IntPtr pAddress = new IntPtr(cogMemoryPlaneArray[iByte % nPlane].Scan0.ToInt64() + iByte / nPlane);
                    Marshal.WriteByte(cogMemoryPlaneArray[iByte % nPlane].Scan0, iByte / nPlane, pBuffer[iByte]);
                }
                cogImage.Copy(CogImageCopyModeConstants.CopyPixels);

                for (int iMemory = 0; iMemory < nPlane; iMemory++)
                {
        ...
        public static CogImage24PlanarColor SetColorImageParallel(byte[] pBuffer, int nWidth, int nHeight, int nPlane=3, bool bRGBOrder = true)
        {
            if (pBuffer == null || pBuffer.Length != nWidth * nHeight * nPlane) return null;
            if (nPlane != 3) return null;
        ...
        public static CogImage24PlanarColor SetColorImageRGB(byte[] pBufferRed, byte[] pBufferGreen, byte[] pBufferBlue, int nWidth, int nHeight)
        {
        ...
        /// <returns></returns>
        public static ICogImage ConvertBinaryImage(CogImage8Grey cogImage)
        {
            ...
            if (cogIPTool.RunStatus.Result == CogToolResultConstants.Accept) return cogIPTool.OutputImage;
            else return null;
        }
        #endregion

        #region Pattern Matching 하기 (PMAlign)
        public static CogPMAlignPattern PatternTrain(CogImage8Grey cogImage, double dOriginX, double dOriginY, bool bAutoLimited, bool bAutoThreshold,
        ...
        public static CogPMAlignPattern PatternTrainAtTopLeftOrigin(CogImage8Grey cogImage, bool bAutoLimited, bool bAutoThreshold,
                                                              double dCoarseLimit = 10.0, double dFineLimit = 3.0, double dThreshold = 30.0)
        {
            ////  Pattern 영역 및 원점 설정
            CogPMAlignPattern cogPattern = new CogPMAlignPattern();
        ...
        public static CognexResult PatternMatching(CogImage8Grey cogImage, ref CogPMAlignPattern cogPattern, double dMatchingThreshold, bool bUseZoneAngle, bool bUseZoneScale,
        ...
        public static PointF PatternMatchingToPoint(CogImage8Grey cogImage, ref CogPMAlignPattern cogPattern, double dMatchingThreshold, bool bUseZoneAngle, bool bUseZoneScale,
                                                                  double dZoneAngleLow = 0.1, double dZoneAngleHigh = 2.0, double dZoneScaleLow = 0.1, double dZoneScaleHigh = 2.0)
        {
            if (cogPattern == null) return new PointF(-10000.0f, -10000.0f);    // Train 된 Pattern 없을시 NULL 처리.

            int nTrainedWidth = 0;
            int nTrainedHeight = 0;
            PointF fPointResult = new PointF();
            ////  PMAlign Parameter 설정
            CogPMAlignRunParams cogPMParam = new CogPMAlignRunParams();
            {
                cogPMParam.ApproximateNumberToFind = 1;
        ...
        public static CogRectangleAffine PatternMatchingToRect(CogImage8Grey cogImage, ref CogPMAlignPattern cogPattern, double dMatchingThreshold, bool bUseZoneAngle, bool bUseZoneScale,
                                                                  double dZoneAngleLow = 0.1, double dZoneAngleHigh = 2.0, double dZoneScaleLow = 0.1, double dZoneScaleHigh = 2.0)
        {
            if (cogPattern == null) return null;    // Train 된 Pattern 없을시 NULL 처리.
        ...
        public static CogImage8Grey EraseRect(CogImage8Grey cogImage, double dCenterX, double dCenterY, double dRectWidth, double dRectHeight)
        {
            ...
            {
                ImageFactory.Draw.PaintCanvas(ref pBitmap, Color.White);
                ImageFactory.Draw.PaintRect(ref pBitmap, (int)dCenterX, (int)dCenterY, (int)dRectWidth, (int)dRectHeight, Color.Black, 1.0);
            }
            ////  새로운 Bitmap으로 Filter 제작 후 Overlap 함
            CogImage8Grey cogFilterImage = new CogImage8Grey(pBitmap);
            return ImageOverlapMin(cogImage, cogFilterImage) as CogImage8Grey;
        }
        ...
        public static CogImage8Grey EraseWithoutRect(CogImage8Grey cogImage, double dCenterX, double dCenterY, double dRectWidth, double dRectHeight)
        {
            ...
            Bitmap pBitmap = new Bitmap(cogImage.Width, cogImage.Height);
            {
                ImageFactory.Draw.PaintCanvas(ref pBitmap, Color.Black);
                ImageFactory.Draw.PaintRect(ref pBitmap, (int)dCenterX, (int)dCenterY, (int)dRectWidth, (int)dRectHeight, Color.White, 1.0);
            }
            ////  새로운 Bitmap으로 Filter 제작 후 Overlap 함
            CogImage8Grey cogFilterImage = new CogImage8Grey(pBitmap);
            return ImageOverlapMin(cogImage, cogFilterImage) as CogImage8Grey;
        ...
        #region Display에 그리기
        public static bool DrawGridToDisplay(CogDisplay fDisplay, int nPartsX, int nPartsY, string strGroup = "")   // nPart가 2이면 Grid 선은 1개, nPart가 3이면 Grid 선은 2개
        {
        ...
        public static CogRectangle DrawROIToDisplay(CogDisplay fDisplay, double dCenterX, double dCenterY, double dWidth, double dHeight, string strGroup = "")
        {
        ...
        public static bool DrawPatternTrainROIToDisplay(CogDisplay fDisplay, double dStartX, double dStartY, double dWidth, double dHeight, string strGroup = "")
        {
            if (fDisplay == null || fDisplay.IsDisposed) return false;
        ...
        public static bool DrawBlobRectToDisplay(CogDisplay fDisplay, CognexResult pResult, int nMinimumPixelCount, string strGroup = "", bool bInteractive = false, bool bDisplayText = false)
        {
            if (fDisplay == null || fDisplay.IsDisposed) return false;
        ...
        public static bool DrawPatternMatchCrossToDisplay(CogDisplay fDisplay, CognexResult pResult, string strGroup = "", bool bInteractive = false, bool bDisplayText = false)
        {
        ...
        public static bool DrawPatternMatchRectToDisplay(CogDisplay fDisplay, CognexResult pResult, string strGroup = "", bool bInteractive = false, bool bDisplayText = false)
        {
            if (pResult == null || pResult.ToolType != eYoonCognexType.PMAlign)
                return false;
        ...
        public static bool DrawCalibrationPointToDisplay(CogDisplay fDisplay, CognexMapping pMapping, string strGroup = "", bool bInteractive = false, bool bDisplayText = false)
        {
        ...
        public static bool DrawRulerToDisplay(CogDisplay fDisplay, double dStartX, double dStartY, double dEndX, double dEndY, double dResolution, string strGroup = "", bool bDisplayText = false)
        {
        ...
        public static CogRectangle DrawRectToDisplay(CogDisplay fDisplay, CogColorConstants nColor, double dCenterX, double dCenterY, double dWidth, double dHeight, bool bInteractive = false, string strGroup = "")
        {
            ////  ROI 사각형 그리기
            CogRectangle fCogRectROI = new CogRectangle();
            {
        ...
        public static CogLineSegment DrawLineToDisplay(CogDisplay fDisplay, CogColorConstants nColor, double dStartX, double dStartY, double dEndX, double dEndY, bool bInteractive = false, string strGroup = "")
        {
            CogLineSegment fCogLine = new CogLineSegment();
            {
        ...
        public static void DrawCrossToDisplay(CogDisplay fDisplay, CogColorConstants nColor, double dPointX, double dPointY, double dLineLength, bool bInteractive = false, string strGroup = "")
        {
            CogLineSegment cogLine_Horz = new CogLineSegment();
            {
                cogLine_Horz.Color = CogColorConstants.Green;
        ...
        public static CogGraphicLabel DrawTextToDisplay(CogDisplay fDisplay, CogColorConstants nColor, double dPosX, double dPosY, string strText, float dSizeFont = 14.0f, bool bInteractive = false, string strGroup = "")
        {
        ...
        public static ICogImage ImageSubstract(ICogImage cogInputImageA, ICogImage cogInputImageB)
        {
            ////  Param 설정
        ...
        public static ICogImage ImageAdd(ICogImage cogInputImageA, ICogImage cogInputImageB)
        {
            ////  Param 설정
        ...
        public static ICogImage ImageOverlapMax(ICogImage cogInputImageA, ICogImage cogInputImageB)
        {
            ////  Param 설정
        ...
        public static ICogImage ImageOverlapMin(ICogImage cogInputImageA, ICogImage cogInputImageB)
        {
            ////  Param 설정
            CogIPTwoImageMinMax cogIPMinMaxParam = new CogIPTwoImageMinMax();
            {
