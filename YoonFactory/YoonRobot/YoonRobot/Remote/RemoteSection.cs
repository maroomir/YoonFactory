...
namespace YoonFactory.Robot.Remote
{
    ...
    public class RemoteSection : IYoonSection<eYoonRemoteType, ParameterRemote>
    {
        ...
        private ParameterRemote GetInitRemoteParam(eYoonRemoteType nKey)
        {
            switch (ParantsType)
            {
                case eYoonRobotType.UR:
                    return URFactory.InitRemoteParameter(nKey);
                case eYoonRobotType.TM:
                    return TMFactory.InitRemoteParameter(nKey);
                default:
        ...
        public eYoonRemoteType KeyOf(int nIndex)
        {
            if (m_pListKeyOrdered == null)
            {
                throw new InvalidOperationException("Cannot call KeyOf(int) on RemoteSection: section was not ordered.");
            }
            if (nIndex < 0 || nIndex > m_pListKeyOrdered.Count)
            {
        ...
        {
            get { return (m_pListKeyOrdered != null) ? (ICollection<eYoonRemoteType>)m_pListKeyOrdered : m_pDicParam.Keys; }
        }
